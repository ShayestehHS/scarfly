...
def tracking_code(update: Update, context: CallbackContext):
    logger.info(f"Function: tracking_code     User:{update.message.from_user.username}")
    update.message.reply_text(text="کد رهگیری ای که ما به شما دادیم را وارد کنید...")
    return STATUS_SECOND


def tracking_code_get_payment_id(update: Update, context: CallbackContext) -> int:
    chat_id = update.message.chat_id
    if not check_user_validation(chat_id):
        update.message.reply_text("شما در ابتدا باید من را فعال کنید تا شما رو بشناسم و از شما دستور بگیرم.")
        activate_user(chat_id, update)
    ...
    subquery = f"SELECT id FROM accounts_user as au WHERE au.chat_id = '{chat_id}'"
    query = f"SELECT tracking_code FROM orders_order " \
            f"INNER JOIN accounts_user au on orders_order.user_id = au.id " \
            f"WHERE orders_order.payment_id = '{payment_id}' " \
            f"AND orders_order.user_id = ({subquery})"
    with Connection() as conn:
    ...
    update.message.reply_text("شناسه ي مرسوله ي شما:"
                              f"\n {results[0]['tracking_code']}")
    update.message.reply_text("\n برای مشاهده ی وضعیت مرسوله خود در وبسایت شرکت پست، روی لینک زیر کلیک کنید:"
                              f"\n https://tracking.post.ir/search.aspx?id={results[0]['tracking_code']}")
    return ConversationHandler.END
...
def main() -> None:
    ...
    tracking_code_conv_handler = ConversationHandler(
        entry_points=[CommandHandler('tracking_code', status)],
        states={
            STATUS_FIRST: [MessageHandler(Filters.text & ~Filters.command, tracking_code)],
            STATUS_SECOND: [MessageHandler(Filters.text & ~Filters.command, tracking_code_get_payment_id)]
        },
        fallbacks=[CommandHandler('cancel', cancel)],
    )
    dispatcher.add_handler(status_conv_handler)
    dispatcher.add_handler(tracking_code_conv_handler)
